...
interface multiBarChartInterface {
    chart: {
        type: string;
        height: number;
        clipEdge: boolean;
...
export const pieChartOptions: pieChartInterface = {
    chart: {
        type: "pieChart",
        height: 500,
        showLabels: true,
...
export const multiBarChartOptions: multiBarChartInterface = {
    chart: {
        type: "multiBarChart",
        height: 450,
        margin: {
...
export class ChartTypes {
    ...
    pieChart(options: pieChartInterface) {
        return fromJS(pieChartOptions).mergeDeep(options).toJS();
    }

    barChart(options: multiBarChartInterface) {

        return fromJS(multiBarChartOptions).mergeDeep(options).toJS();
    }

    lineChart(options: lineChartInterface) {
